...
typedef vector<float> term_grade;
...
void output_courses_id(student_courses courses, int term);
...
float calculate_total_average_grade(term_grade term_average_grade);
void pass_courses(student_grades &grades, student_courses next_term_courses);
...
int main(int argc, char *argv[]) {
    ...
    term_grade term_average_grades;
    term_average_grades.push_back(calculate_average_grade(grades, courses));

    for (int term = 1; !all_allowed_courses.empty(); term++) {

        all_allowed_courses.clear();
        all_allowed_courses = find_allowed_courses(courses, grades);
        sort(all_allowed_courses.begin(), all_allowed_courses.end(), sort_course_by_unit);
        sort_name_unit(all_allowed_courses);
        float total_average_grade = calculate_total_average_grade(term_average_grades);
        student_courses next_term_courses = suggested_courses(all_allowed_courses, total_average_grade);
        sort(next_term_courses.begin(), next_term_courses.end(), sort_course_by_name);
        pass_courses(grades, next_term_courses);
        if (!all_allowed_courses.empty()) output_courses_id(next_term_courses, term);
        total_average_grade += ((total_average_grade * 5) / 100);
        term_average_grades.push_back(total_average_grade);
    }
...
void output_courses_id(student_courses courses, int term) {
    ...
    string file_name = "semester" + to_string(term) + ".sched";
    ofstream semester(file_name);

    for (int index = 0; index < courses.size(); ++index) {
        semester << courses[index].id << endl;
    }
}
...
float calculate_total_average_grade(term_grade term_average_grade) {
    ...
    float sum = 0;
    for (int index = 0; index < term_average_grade.size(); index++) {
        sum += term_average_grade[index];
    }
    
    return sum / term_average_grade.size();
}

void pass_courses(student_grades &grades, student_courses next_term_courses) {

    for (int index = 0; index < next_term_courses.size(); ++index) {

        course_grade new_grade;
        new_grade.id = next_term_courses[index].id;
        new_grade.grade = PASS_GRADE;
